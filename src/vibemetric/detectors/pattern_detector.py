...
class PatternDetector:
    ...
    def analyze_commit_message(self, message: str) -> DetectionSignal:
        ...
        if bullet_count >= 3:
            ...
            evidence.append(f"List-style commit with {bullet_count} bullet points (STRONG AI indicator)")
        elif bullet_count >= 1:
    ...
    def analyze_pr_description(self, description: str) -> DetectionSignal:
        ...
        if re.search(r'^This PR (introduces|adds|implements|fixes|updates|refactors)', 
        ...
        if pr_links >= 2:
            pattern_scores.append(70.0)
            evidence.append(f"Multiple PR cross-references ({pr_links} links - STRONG AI indicator)")
        elif pr_links >= 1:
            pattern_scores.append(50.0)
        ...
        if technical_sections >= 2:
            ...
            evidence.append(f"Extensive technical documentation ({technical_sections} sections - VERY AI-like)")
    ...
    def analyze_code(self, code: str, language: str = "python") -> DetectionSignal:
        ...
        if comment_score > self.pattern_threshold:
            ...
            evidence.append(
                f"AI-style comments detected ({len(comment_matches)} matches)"
            )
        ...
        if hallucinations:
            ...
            evidence.append(
                f"Hallucination patterns detected ({len(hallucinations)} instances)"
            )
        ...
        if ai_patterns:
            ...
            evidence.append(
                f"AI code patterns detected ({len(ai_patterns)} instances)"
            )
