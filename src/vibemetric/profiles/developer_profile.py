...
class DeveloperProfile:
    ...
    def format_terminal(self) -> str:
        ...
        if self.total_commits > 0:
            lines.append(
                f"  AI-Assisted Commits: {self.ai_assisted_commits} ({self.ai_commit_percentage:.1f}%)"
            )
...
class DeveloperProfiler:
    ...
    def generate_profile(
        self, author: Optional[str] = None, email: Optional[str] = None
        ...
        if velocity_metrics["baseline_velocity"] > 0:
            velocity_improvement = (
                velocity_metrics["current_velocity"] / velocity_metrics["baseline_velocity"] - 1.0
            ) * 100
        ...
        for artifact in artifacts:
            ...
            if author and author in artifact.authors:
                ...
                try:
                    ...
                    commits = list(
                        self.artifact_detector.repo.iter_commits(paths=str(rel_path), reverse=True)
                    )
