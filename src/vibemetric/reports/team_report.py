...
class TeamReport:
    ...
    def format_terminal(self) -> str:
        ...
        ai_tool_pct = (
            (self.ai_tool_users / self.total_developers * 100) if self.total_developers > 0 else 0
        )
        lines.append(f"  AI Tool Users: {self.ai_tool_users} ({ai_tool_pct:.1f}%)")

        shadow_pct = (
            (self.shadow_ai_users / self.total_developers * 100) if self.total_developers > 0 else 0
        )
        lines.append(f"  Shadow AI Users: {self.shadow_ai_users} ({shadow_pct:.1f}%)")

        human_pct = (
            (self.human_only_developers / self.total_developers * 100)
            if self.total_developers > 0
            else 0
        )
        ...
        if self.tool_distribution:
            ...
            for tool, count in sorted(
                self.tool_distribution.items(), key=lambda x: x[1], reverse=True
            ):
        ...
        if metrics.get("developers_with_velocity_data", 0) > 0:
            ...
            if metrics["developers_with_positive_impact"] > 0:
                lines.append(
                    f"  Developers with Positive Impact: {metrics['developers_with_positive_impact']}"
                )
            if metrics["developers_with_negative_impact"] > 0:
                lines.append(
                    f"  Developers with Negative Impact: {metrics['developers_with_negative_impact']}"
                )
            if (
...
class TeamReporter:
    ...
    def generate_report(self, min_commits: int = 1) -> TeamReport:
        ...
        shadow_ai_users = len(
            [p for p in profiles if not p.ai_tools and p.ai_commit_percentage > 50.0]
        )
        ...
        ai_adoption_rate = (
            ((ai_tool_users + shadow_ai_users) / total_developers * 100)
            if total_developers > 0
            else 0.0
        )
        ...
        top_ai_users = sorted(
            profiles, key=lambda p: (p.ai_commit_percentage, p.total_commits), reverse=True
        )[:20]
    ...
    def _calculate_productivity_metrics(self, profiles: List[DeveloperProfile]) -> Dict[str, Any]:
        ...
        ai_assisted_percentage = (
            (ai_assisted_commits / total_commits * 100) if total_commits > 0 else 0.0
        )
