...
class PRAnalysisResult:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "files_analysis": {
                ...
                "high_ai_files": [
                    {"filename": f.filename, "ai_score": f.ai_score, "additions": f.additions}
    ...
    def format_terminal(self) -> str:
        ...
        if self.commits:
            ...
            lines.append(
                f"  AI-Style Commits: {self.ai_style_commits}/{len(self.commits)} ({ai_pct:.1f}%)"
            )
...
class PRAnalyzer:
    ...
    def analyze_pr_from_data(
        ...
        for file_data in files_data:
            ...
            if pr_file.content and self._should_analyze_file(pr_file.filename):
                file_signal = self.pattern_detector.analyze_code(
                    pr_file.content, language=self._detect_language(pr_file.filename)
                )
        ...
        if result.commits:
            result.avg_commit_ai_score = sum(c.ai_score for c in result.commits) / len(
                result.commits
            )
